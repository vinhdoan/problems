...
To scan most common ports,
use: python port_scanner.py example.com
''' + line + "\n"
...
parser.add_argument('startport',
                    metavar='P1',
                    nargs='?',
                    help='Start scanning from this port')
parser.add_argument('endport',
                    metavar='P2',
                    nargs='?',
                    help='Scan until this port')
...
# The flag is set, that means the user did not provide any ports as argument
if (flag):
...
def check_port(host, port, result=1):
    ...
    try:
        ...
        # Setting socket timeout so that the socket does not wait forever
...
try:
        ...
        if flag:  # The flag is set, means the user did not give any port range
            for p in sorted(common_ports):  # So we will scan the common ports.
                ...
                p = int(p)
                print p,
                # call the function to connect to the port
                response = check_port(host, p)
                if response == 0:  # The port is open
                    open_ports.append(p)  # append it to the list of open ports
                    # if not p == end_port:
                    # This is just used to clear the port number displayed.
                    # This is not important at all
                    sys.stdout.write('\b' * len(str(p)))
        else:
            # The user did provide a port range,
            # now we have to scan through that range
        ...
        # Calculating the total time used to scan
...
# This is used in case the  user press "Ctrl+C",
#  it will show the following error instead of a python's scary error
